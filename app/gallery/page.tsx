  ...
  return (
    <>
      ...
      <section style={{ position: 'relative', height: '50vh', minHeight: '380px', background: 'var(--charcoal)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
        <Image src="/about.png" alt="Gallery" fill style={{ objectFit: 'cover', opacity: 0.2 }} />
      ...
      <section style={{ padding: '60px 0 100px', background: 'var(--warm-white)' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 32px' }}>
          {galleryImages.length === 0 ? (
            <p style={{ color: 'var(--warm-gray)', textAlign: 'center' }}>Gallery photos coming soon.</p>
          ) : (
            <div style={{ columns: '3 280px', gap: '20px' }}>
              {galleryImages.map(({ id, img, alt, label }) => (
                <div key={id} className="card-hover img-overlay" style={{ borderRadius: '12px', overflow: 'hidden', marginBottom: '20px', breakInside: 'avoid', cursor: 'pointer', position: 'relative' }}>
                  <Image src={img} alt={alt} width={600} height={400} style={{ width: '100%', height: 'auto', display: 'block' }} />
